...
// Fix: Redefine interface to match the expected name 'AIStudio' to avoid subsequent property declaration conflicts
interface AIStudio {
...
export const ServiceOrderDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (showPartModal && catalog.length === 0) {
      mockData.getCatalog().then(setCatalog).catch(() => setErrorMessage("Erro ao carregar catálogo de peças."));
    }
  }, [showPartModal]);
  ...
  const handleGenerateAISummary = async () => {
    ...
    try {
      ...
      // Tratamento amigável de erros de API
      const msg = e.message || "Erro desconhecido.";
      if (msg.includes("API_KEY_MISSING") || msg.includes("Requested entity was not found")) {
         setErrorMessage("CHAVE DE API NÃO DETETADA OU INVÁLIDA EM PRODUÇÃO. VERIFIQUE AS VARIÁVEIS DE AMBIENTE DA VERCEL.");
      } else {
         setErrorMessage(msg);
      }
    } finally {
    ...
    <div className="max-w-4xl mx-auto pb-44 relative px-1 sm:px-0 space-y-4">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
        {activeTab === 'info' && (
          <div className="space-y-3">
            ...
            {/* Outros cartões INFO mantidos para brevidade (cliente, planeamento, log) */}
          </div>
        )}
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] w-[92%] max-w-lg bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-[#9d1c24] dark:border-[#9d1c24]/60 shadow-[0_12px_40px_rgba(157,28,36,0.15)] rounded-full p-1.5 flex items-center justify-around transition-all animate-in slide-in-from-bottom-10 duration-500">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as TabType)} className={`flex-1 flex flex-col items-center justify-center py-2.5 rounded-full transition-all gap-1 ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'}`}><tab.icon size={18} /><span className="text-[7px] font-black uppercase tracking-widest">{tab.label}</span></button>
        ))}
      </div>

      {showValidationErrorModal && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"><div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-sm overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 transition-colors"><div className="p-8 text-center"><div className="w-16 h-16 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner"><ShieldAlert size={32}/></div><h3 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight mb-2">Dados em Falta</h3><p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed mb-6 uppercase">Para concluir a OS, deve preencher os seguintes campos obrigatórios:</p><div className="space-y-2 mb-8">{missingFields.map((field, idx) => (<div key={idx} className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 p-3 rounded-xl text-[10px] font-black text-red-600 dark:text-red-400 uppercase tracking-widest">{field}</div>))}</div><button onClick={() => setShowValidationErrorModal(false)} className="w-full py-4 bg-slate-900 dark:bg-blue-600 text-white font-black text-[10px] uppercase rounded-2xl shadow-xl active:scale-95 transition-all">ENTENDIDO, VOU PREENCHER</button></div></div></div>
      )}
