...
import { ArrowLeft, FileText, Loader2, Check, AlertCircle, Box, MapPin, Hash, Trash2, Plus, Search, User } from 'lucide-react';
...
const EditDelivery: React.FC = () => {
  ...
  const [loadingAddressType, setLoadingAddressType] = useState<'CR' | 'PM' | 'other'>('other');
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    try {
      const clientsData = await mockData.getClients();
      ...
      if (id) {
        ...
        if (data.loading_address === 'Real Frio - CR') {
          setLoadingAddressType('CR');
        } else if (data.loading_address === 'Real Frio - PM') {
          setLoadingAddressType('PM');
        } else {
          setLoadingAddressType('other');
        }
        ...
        // Tentar recuperar IDs de cliente/estabelecimento das notas
        if (data.notes) {
          try {
            const notes = JSON.parse(data.notes);
            if (notes.client_id) {
              setSelectedClientId(notes.client_id);
              const ests = await mockData.getEstablishmentsByClient(notes.client_id);
              setEstablishments(ests);
              if (notes.establishment_id) {
                setSelectedEstId(notes.establishment_id);
              }
            }
          } catch (e) {
            console.warn("Erro ao processar notas da entrega:", e);
          }
        }
      }
    } catch (err: any) {
  ...
  const handleLoadingAddressTypeChange = (type: 'CR' | 'PM' | 'other') => {
    ...
    if (type === 'CR') {
      ...
      // Se mudar para 'other' e o valor atual não for CR ou PM, mantém. Caso contrário limpa.
      if (formData.loading_address === 'Real Frio - CR' || formData.loading_address === 'Real Frio - PM') {
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                >
                  <option value="">-- SELECIONAR CLIENTE OU ALTERAR MANUALMENTE --</option>
                  {clients.map(c => (
